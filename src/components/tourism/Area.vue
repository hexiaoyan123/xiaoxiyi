...
<div class="container area">
  ...
  <div style="min-height: 101%;">
    <div class="top-img" v-if="info.imageUrl">
      <figure class="img">
    ...
    <div class="card-list-name" v-show="resTravelAreaList.length > 0">
      <div class="info-name">
        <div class="info-name-left">区域推荐</div>
        <div class="info-name-right"></div>
      </div>
    </div>
    <div class="card-list clearfix">
      <div class="card-item" v-for="(item, index) in resTravelAreaList" :key="index" @click="openArea(item.id)">
    ...
    <div class="card-list-name" v-show="resScenicSpotList.length > 0">
      <div class="info-name">
        <div class="info-name-left">景点推荐</div>
        <div class="info-name-right"></div>
      </div>
    </div>
    <div class="card-list clearfix">
      <div class="card-item" v-for="(item, index) in resScenicSpotList" :key="index" @click="openSpot(item.id)">
        <div class="card-item-img">
          <figure class="img">
            <img v-if="item.imageUrl" :src="imgIp + item.imageUrl" alt="">
          </figure>
        </div>
        <div class="card-item-name unoverflow">
          {{item.name}}
        </div>
      </div>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      info: {}, // 区域详情
      resTravelAreaList: [], // 区域列表
      resScenicSpotList: [], // 景点推荐
      strategysList: [], // 攻略推荐
      taps: [], // 标签列表
    }
  },
  ...
  methods: {
    // 打开城市
    openArea (id) {
      this.$router.push('/tourism/area/'+ id)
    },
    // 打开景点
    openSpot (id) {
    ...
    getArea () {
      let vm = this;
      this.$http.get('/panda-travel-api/v1/getTravelArea/' + this.id, this.$init.channel.app).then(function (response) {
        if("1000" == response.data.code){
          if(response.data.result){
            vm.info = response.data.result;
            if(response.data.result.resTravelAreaList){
              vm.resTravelAreaList = response.data.result.resTravelAreaList;
            }
            if(response.data.result.resScenicSpotList){
              vm.resScenicSpotList = response.data.result.resScenicSpotList;
            }
            // 标签处理
