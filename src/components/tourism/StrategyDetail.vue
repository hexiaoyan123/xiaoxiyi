...
<div class="container area">
  ...
  <div style="min-height: 101%;">
    ...
    <div class="strategy-info">
      <div class="strategy-info-left">浏览人数: <em>{{info.browseNum}}</em></div>
      <div class="strategy-info-right">
        作者: <em>{{info.author}}</em>
      </div>
    </div>
    <div class="taps clearfix" v-show="taps.length > 0">
      <div class="tap" v-for="(item, index) in taps" :key="index">{{item}}</div>
    </div>
    <div class="strategy-cont rich-text" v-html="info.description"></div>
  </div>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    getStrategy () {
      let vm = this;
      this.$http.get('/panda-travel-api/v1/getTravelStrategy/'+ this.id, this.$init.channel.app).then(function (response) {
